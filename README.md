...
**Подготовка файлов подключения:**

- Обязательно отредактируйте файл connect.php, который следует по пути (директории): "C:\OpenServer\domains\studentsActivity\src\config\connect.php")
P.s. Вместо диска C: может быть любой другой диск (например, D: или F:)
- Редактировать значения через IDE (например, visual studio code) или notepad++. Скорее всего, потребуется отредактировать лишь строку 11 (public static $host = '127.0.0.1:3306';)
Если порт отличается от того, что указан в Open Server Panel (OSPANEL), то замените его в коде на необходимый. Проверить правильный порт можно в параметрах OSPANEL в разделе Server (Сервер).

![То, что нужно будет отредактировать, если отличается](media/image6.png)
